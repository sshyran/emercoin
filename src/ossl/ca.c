...
int opt_md(const char *name, const EVP_MD **mdp)
{
	*mdp = EVP_get_digestbyname(name);
	if (*mdp != NULL)
		return 1;
	BIO_printf(bio_err, "openssl: Unrecognized flag %s\n", name);
	return 0;
}
...
int ca_main(char *configfile, const char *infile, const char* outfile)
{
	...
	const char *spkac_file = NULL, *ss_cert_file = NULL;
    const char *extensions = NULL, *extfile = NULL, *passinarg = NULL;
	char *outdir = NULL, *rev_arg = NULL, *ser_status = NULL;
	...
	int default_op = 1, doupdatedb = 0, ext_copy = EXT_COPY_NONE;
    int keyformat = FORMAT_PEM, multirdn = 0, notext = 0, output_der = 0;
	int ret = 1, email_dn = 1, verbose = 0, gencrl = 0, dorevoke = 0;
    ...
    if ((outdir == NULL) && (req)) {
        ...
        if (app_isdir(outdir) <= 0) {
			BIO_printf(bio_err, "openssl: %s is not a directory\n", outdir);
            perror(outdir);
            goto end;
        }
#endif
