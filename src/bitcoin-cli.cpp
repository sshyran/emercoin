...
static void SetupCliArgs()
{
    ...
    gArgs.AddArg("-legacyrpc", "Use json_spirit library to decode/encode input/output (default: true)", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
}
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  emercoin-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     emercoin-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     emercoin-cli [options] help                List commands\n"
                "or:     emercoin-cli [options] help <command>      Get help for a command\n";
