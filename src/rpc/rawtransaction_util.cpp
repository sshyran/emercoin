...
UniValue SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, const UniValue& hashType)
{
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); i++) {
        ...
        if (!VerifyScript(txin.scriptSig, prevPubKey, mtx.nVersion, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&txConst, i, amount), &serror)) {
