...
UniValue getcheckpoint(const JSONRPCRequest& request)
{
    ...
    const CBlockIndex* pindexPendingCheckpoint = LookupBlockIndex(CheckpointsSync::hashPendingCheckpoint);
    if (pindexPendingCheckpoint) {
        result.pushKV("pendingsynccheckpoint", CheckpointsSync::hashPendingCheckpoint.ToString());
        result.pushKV("pendingheight", pindexPendingCheckpoint->nHeight);
        result.pushKV("pendingtimestamp", FormatISO8601DateTime(pindexPendingCheckpoint->GetBlockTime()));
    }
