...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    for (const auto& txout : tx.vout)
    {
        if (txout.IsEmpty() && (!tx.IsCoinBase()) && (!tx.IsCoinStake()))
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "empty-txout");
    ...
    for (const CTxIn& txin : tx.vin) {
        if (txin.prevout.hash == randpaytx) {
            if (found)
                return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "multiple-randpay-utxo");
            found = true;
        }
    }
