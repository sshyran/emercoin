...
bool Consensus::CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); ++i) {
        ...
        if (coin.IsCoinBase() || coin.IsCoinStake()) {
            ...
            if (nSpendHeight - coin.nHeight < cnbMaturity)
                return state.Invalid(ValidationInvalidReason::TX_PREMATURE_SPEND, false, REJECT_INVALID, "bad-txns-premature-spend-of-coinbase/coinstake",
    ...
    if (tx.IsCoinStake()) {
        ...
        if (!GetEmc7POSReward(tx, inputs, nLimit))
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "cannot-read-reward");
