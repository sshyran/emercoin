...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    if (solved && (whichType == TX_SCRIPTHASH || whichType == TX_NAME_SCRIPTHASH))
    {
        ...
        solved = solved && SignStep(provider, creator, subscript, result, whichType, SigVersion::BASE, sigdata) && whichType != TX_SCRIPTHASH && whichType != TX_NAME_SCRIPTHASH;
        P2SH = true;
    }
    ...
    // Test solution
    sigdata.complete = solved && VerifyScript(sigdata.scriptSig, fromPubKey, fName ? NAMECOIN_TX_VERSION : 0, &sigdata.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, creator.Checker(), nullptr);
...
bool IsSolvable(const SigningProvider& provider, const CScript& script, int32_t nVersion)
{
