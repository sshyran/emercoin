...
extern arith_uint256 nMinimumChainTrust;
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW = true, bool fCheckMerkleRoot = true, bool fCheckSign = true) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
public:
     ...
     * per index entry (nStatus, nChainTrust, nTimeMax, etc.) as well as peripheral
    ...
    CBlockIndex* AddToBlockIndex(const CBlockHeader& block, bool bSetAsProofOfstake) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
public:
    ...
    DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool fWriteNames=false);
    bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex,
                      CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false, bool fWriteNames=false) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
