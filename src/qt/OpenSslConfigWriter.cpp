...
# сертификата
# (в шестнадцатиричном формате)
certificate            = CA-RSA/emcssl_ca.crt        # Файл сертификата CA
private_key            = CA-RSA/emcssl_ca.key        # Файл закрытого ключа CA
...
0.organizationName_default	= EmerCoin Foundation
...
QString OpenSslConfigWriter::checkAndWrite() {
	...
	if(    !writeIfAbsent("CA-EC/ca.config", ecConfig, error)
		|| !writeIfAbsent("CA-EC/emcssl_ca.key", ecKey, error)
		|| !writeIfAbsent("CA-EC/emcssl_ca.crt", ecCrt, error)
		|| !writeIfAbsent("CA-RSA/ca.config", rsaConfig, error)
		|| !writeIfAbsent("CA-RSA/emcssl_ca.key", rsaKey, error)
		|| !writeIfAbsent("CA-RSA/emcssl_ca.crt", rsaCrt, error)
