...
class Args2: public QStringList {
	protected:
		QVector<char*> _argv;
		QList<QByteArray> _args;
	public:
		Args2(const QStringList & args): QStringList(args) {
		}
		int argc()const {
			return count();
		}
...
bool OpenSslExecutable::encryptInfocardAes(const QString& fileIn, const QString & outFile, const QString & pass) {
	...
	const char* chError = enc_main(_args(fileIn), "aes-256-cbc", _args(pass), _args(outFile));
	QString error = chError;
	if(!error.isEmpty()) {
		log(tr("Error - openssl pkcs12 returned %1").arg(error));
		return false;
	}
	return true;
}
