...
static void UpdateMempoolForReorg(DisconnectedBlockTransactions& disconnectpool, bool fAddToMempool) EXCLUSIVE_LOCKS_REQUIRED(cs_main, ::mempool.cs)
{
    ...
    while (it != disconnectpool.queuedTx.get<insertion_order>().rend()) {
        ...
        if (!fAddToMempool || (*it)->IsCoinBase() ||
            !AcceptToMemoryPool(mempool, stateDummy, *it, nullptr /* pfMissingInputs */, true /* bypass_limits */, 0 /* nAbsurdFee */)) {
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        if (m_pool.GetConflictTx(txin.prevout)) {
    ...
    if (isNameTx && !hooks->IsNameFeeEnough(ptx, nFees))
        return state.Invalid(ValidationInvalidReason::TX_MEMPOOL_POLICY, false, REJECT_INSUFFICIENTFEE, "name fee is below minimum");
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    bool validForFeeEstimation = !bypass_limits && IsCurrentForFeeEstimation() && m_pool.HasNoInputsOf(tx);
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if((block.GetBlockVersion() < 2 && nHeight >= consensusParams.BIP34Height) ||
       ...
       (block.GetBlockVersion() < 7 && nHeight >= consensusParams.SegwitHeight))
