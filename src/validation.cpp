...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (tx.IsCoinBase() || tx.IsCoinStake())
        return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "coinbase/coinstake");
    ...
    if (isNameTx && !hooks->CheckPendingNames(ptx))
        return state.Invalid(ValidationInvalidReason::TX_PREMATURE_SPEND, false, REJECT_NONSTANDARD, "name-op-on-pending-name");
    ...
    //emcTODO add minimum fee check here
...
static bool AcceptToMemoryPoolWithTime(const CChainParams& chainparams, CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx,
                        ...
                        bool bypass_limits, const CAmount nAbsurdFee, bool test_accept, bool fRandPayCheck=false) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    std::vector<COutPoint> coins_to_uncache;
    MemPoolAccept::ATMPArgs args { chainparams, state, pfMissingInputs, nAcceptTime, plTxnReplaced, bypass_limits, nAbsurdFee, coins_to_uncache, test_accept, fRandPayCheck };
                        ...
                        bool bypass_limits, const CAmount nAbsurdFee, bool test_accept, bool fRandPayCheck)
{
    ...
    return AcceptToMemoryPoolWithTime(chainparams, pool, state, tx, pfMissingInputs, GetTime(), plTxnReplaced, bypass_limits, nAbsurdFee, test_accept, fRandPayCheck);
}
...
void static InvalidChainFound(CBlockIndex* pindexNew) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    LogPrintf("%s: invalid block=%s  height=%d  log2_trust=%.8g  date=%s  moneysupply=%s\n", __func__,
      ...
      log(pindexNew->nChainTrust.getdouble())/log(2.0), FormatISO8601DateTime(pindexNew->GetBlockTime()), FormatMoney(pindexNew->nMoneySupply));
    CBlockIndex *tip = ::ChainActive().Tip();
    assert (tip);
...
bool CScriptCheck::operator()() {
    ...
    return VerifyScript(scriptSig, scriptPubKey2, ptxTo->nVersion, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, m_tx_out.nValue, cacheStore, *txdata), &error);
}
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool fWriteNames)
{
    bool fClean = true;
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                if (!is_spent || tx.vout[o] != coin.out || pindex->nHeight != coin.nHeight || is_coinbase != coin.fCoinBase || is_coinstake != coin.fCoinStake || tx.nTime != coin.nTime) {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck, bool fWriteNames)
{
    ...
    if (!CheckMinTxOut(block, IsWitnessEnabled(::ChainActive().Tip(), chainparams.GetConsensus())))
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    LogPrintf("%s: new best=%s height=%d version=0x%08x log2_trust=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)%s moneysupply=%s\n", __func__,
...
bool CChainState::DisconnectTip(CValidationState& state, const CChainParams& chainparams, DisconnectedBlockTransactions *disconnectpool)
{
    ...
    int64_t nStart = GetTimeMicros();
    {
        ...
        if (DisconnectBlock(block, pindexDelete, view, true) != DISCONNECT_OK)
...
CBlockIndex* BlockManager::AddToBlockIndex(const CBlockHeader& block, bool bSetAsProofOfstake)
{
    AssertLockHeld(cs_main);
 ...
 */
static bool ContextualCheckBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, bool fCheckSign=true)
{
    ...
    if (block.vtx[0]->GetValueOut() > powLimit)
        return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-cb-amount", "coinbase pays too much");
    ...
    if (block.IsProofOfStake() && (block.vtx[0]->vout.size() != (fHaveWitness ? 2 : 1) || !block.vtx[0]->vout[0].IsEmpty()))
        return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "pos-cb-not-empty", strprintf("%s : coinbase output not empty for proof-of-stake block", __func__));
    ...
    bool fV7Enabled = IsWitnessEnabled(pindexPrev, consensusParams);
    ...
    if (!CheckMinTxOut(block, fV7Enabled))
        return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "txout-too-low", strprintf("%s : txout.nValue below minimum", __func__));

    return true;
}

bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
    uint256 hash = block.GetHash();
...
bool ProcessNewBlockHeaders(int32_t& nPoSTemperature, const uint256& lastAcceptedHeader, const std::vector<CBlockHeader>& headers, CValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex, CBlockHeader *first_invalid)
{
    if (first_invalid != nullptr) first_invalid->SetNull();
    {
        ...
        for (const CBlockHeader& header : headers) {
            ...
            pindex = nullptr;
            bool accepted = g_blockman.AcceptBlockHeader(header, header.nFlags & BLOCK_PROOF_OF_STAKE, state, chainparams, &pindex);
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    bool accepted_header = m_blockman.AcceptBlockHeader(block, block.IsProofOfStake(), state, chainparams, &pindex);
    CheckBlockIndex(chainparams.GetConsensus());
    ...
    if (fCheckPoS && !ppcoinContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
        state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-pos", "proof of stake is incorrect");
        return error("%s: %s", __func__, FormatStateMessage(state));
    }
    ...
    if (!fInitialDownload && m_chain.Tip() == pindex->pprev)
...
bool CChainState::LoadChainTip(const CChainParams& chainparams)
{
    ...
    tip = m_chain.Tip();
    LogPrintf("Loaded best chain: hashBestChain=%s height=%d date=%s progress=%f trust=%s\n",
        ...
        while (num--) {
            ...
            if (nTime + nExpiryTimeout > nNow) {
                ...
                AcceptToMemoryPoolWithTime(chainparams, pool, state, tx, nullptr /* pfMissingInputs */, nTime,
                                           ...
                                           false /* test_accept */); //emcTODO - is randpay check needed here?
                if (state.IsValid()) {
                    ++count;
                } else {
...
bool SignBlock(CBlock& block, const CWallet& keystore)
{
    ...
    bool fV7Enabled = IsWitnessEnabled(::ChainActive().Tip(),  Params().GetConsensus());
...
void CleanMapBlockIndex() {
    ...
    for (auto& it : vit) {
        if ((*it).second.pindex->pprev == nullptr) {
            BlockMap::iterator mi = g_blockman.m_block_index.find((*it).first);
            assert(mi != g_blockman.m_block_index.end());  // it should exist because no other function deletes specific elemets
            delete (*mi).second;
            g_blockman.m_block_index.erase(mi);
