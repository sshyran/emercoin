 ...
 */
static const char* BITCOIN_PID_FILENAME = "emercoin.pid";
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-alertpubkey=<hex>", "Set alert public key. 0 - disable, 1 - default key, hex string - custom key", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#endif
    gArgs.AddArg("-assumevalid=<hex>", strprintf("If this block is in the chain assume that it and its ancestors are valid and potentially skip their script verification (0 to verify all, default: %s, testnet: %s)", defaultChainParams->GetConsensus().defaultAssumeValid.GetHex(), testnetChainParams->GetConsensus().defaultAssumeValid.GetHex()), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    gArgs.AddArg("-checkpointpubkey=<hex>", "Set checkpoint public key. 0 - disable, 1 - default key, hex string - custom key", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
//        }
        return InitError("Prune mode is disabled in emercoin.");
    }
    ...
    if (gArgs.IsArgSet("-minimumchainwork")) {
        const std::string minChainTrustStr = gArgs.GetArg("-minimumchainwork", "");
        if (!IsHexNumber(minChainTrustStr)) {
            return InitError(strprintf("Invalid non-hex (%s) minimum chain work value specified", minChainTrustStr));
        }
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    string ck = gArgs.GetArg("-checkpointpubkey", "1");
    if (ck == "1")
    {
    ...
    if (gArgs.GetBoolArg("-emcdns", false)) {
        int port = gArgs.GetArg("-emcdnsport", EMCDNS_PORT);
        int verbose = gArgs.GetArg("-emcdnsverbose", 1);
        if (port <= 0)
            port = EMCDNS_PORT;
        string suffix  = gArgs.GetArg("-emcdnssuffix", "");
        string bind_ip = gArgs.GetArg("-emcdnsbindip", "");
        string allowed = gArgs.GetArg("-emcdnsallowed", "");
        string localcf = gArgs.GetArg("-emcdnslocalcf", "");
        string enums   = gArgs.GetArg("-enumtrust", "");
        string tf      = gArgs.GetArg("-enumtollfree", "");
        uint32_t dapzs = gArgs.GetArg("-dapsize", 0);
        uint32_t dapth = gArgs.GetArg("-daptreshold", EMCDNS_DAPTRESHOLD);
