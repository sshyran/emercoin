...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " emercoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  emercoin-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoin transaction\n" +
            "or:     emercoin-tx [options] -create [commands]   Create hex-encoded bitcoin transaction\n" +
            "\n";
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    UniValue prevtxsObj = registers["prevtxs"];
    {
        for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
            ...
            if ((scriptPubKey.IsPayToScriptHash(coins->nVersion) || scriptPubKey.IsPayToWitnessScriptHash()) &&
